...
import { doc, setDoc, query, collection, where, getDocs, serverTimestamp, updateDoc, onSnapshot } from "firebase/firestore";
...
export default function Home() {
  ...
  useEffect(() => {
    // Listen to global system settings to conditionally show/hide Google Auth
    const unsub = onSnapshot(doc(db, "systemSettings", "global"), (snap) => {
      if (snap.exists()) {
        const data = snap.data();
        setAllowGoogleAuth(data.allowGoogleAuth === true);
      } else {
        setAllowGoogleAuth(false);
      }
    }, (err) => {
      console.warn("Could not load systemSettings for Google Auth toggle:", err);
      setAllowGoogleAuth(false);
    });
    return () => unsub();
  }, []);
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-4 sm:p-6 w-full my-auto overflow-x-hidden pt-[max(2rem,calc(1.5rem+env(safe-area-inset-top,0px)))] pb-[max(2rem,calc(1.5rem+env(safe-area-inset-bottom,0px)))]">
